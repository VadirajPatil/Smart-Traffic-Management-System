...
class onlyDisplay{		    	// user interface and authentication process
	public:
		void authentication();
		void load();
		void menu();		
...
class features{		// features of the Smart Traffic Management System
	public:
		void record_veh();
		void booths();
		void helpline();
...
class vehData {			// data of a vehicle
	private:
...
void vehData::header() {
	cout << endl;
	cout<<setw(10)<<"Vehicle No"<<setw(10)<<"Vehicle Type"<<setw(10)<<"Owner Name"<<setw(10)<<"Phone Number"<<setw(10)<<"Seater"<<setw(10)<<"Generation"<<setw(10)<<"Fuel"<<setw(10)<<"RTO No"<<setw(10)<<"Issued Date"<<setw(10)<<"Address\n";
	int i;
	for(i=0; i<=170; i++)
		cout << "_";
	cout << endl;
...
void vehData::show_data() {
	cout<<"\n\n"<<setw(10)<<veh_no<<setw(10)<<veh_type<<setw(10)<<owner_name<<setw(10)<<owner_no<<setw(10)<<seater<<setw(10)<<generation<<setw(10)<<des_pat<<setw(10)<<RTO_no<<setw(10)<<issued_date<<setw(10)<<address;
}
...
void onlyDisplay::menu() {
	...
	if(sound==0){
		PlaySoundW(L"start.wav", NULL, SND_SYNC);
		sound++;
	}
